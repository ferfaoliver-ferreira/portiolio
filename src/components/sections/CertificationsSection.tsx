...
export default function CertificationsSection() {
  const parseDateString = (d?: string) => {
    ...
    // dd de month de yyyy
    const m = s.match(/(\d{1,2})\s+de\s+([a-zçãéó]+)\s+de\s+(\d{4})/);
    if (m) {
    ...
    // month yyyy
    const m2 = s.match(/([a-zçãéó]+)\s+(\d{4})/);
    if (m2) {
      const mon = months[m2[1]] ?? 0;
      const year = Number(m2[2]);
      return new Date(year, mon, 1).getTime();
    }
    // year only
    const m3 = s.match(/(\d{4})/);
    if (m3) return new Date(Number(m3[1]), 0, 1).getTime();
  ...
  const certSort = (a: any, b: any) => {
    ...
    if (a.type === "certificacao" && b.type !== "certificacao") return -1;
    if (a.type !== "certificacao" && b.type === "certificacao") return 1;
    return 0;
  };
  return (
        ...
        <Tabs defaultValue="todos" className="w-full">
          ...
          <TabsContent value="todos">
            <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-4">
              {[...certifications].sort(certSort).map((cert) => (
